...
SUPABASE_URL = "https://wkiungalyergysjbhsxz.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndraXVuZ2FseWVyZ3lzamJoc3h6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQwMDE0ODEsImV4cCI6MjA4OTU3NzQ4MX0.MizR5Qaw8J6j_m7784UuFzv9V8_RDZs8jM69QeWXmO0"
...
def save_users(users):
    ...
    for name, data in users.items():
        save_user(name, data["role"], data["categories"])
...
def init_session():
    ...
    if "production_data" not in st.session_state:
        st.session_state.production_data = load_production().to_dict('records')
    ...
    with st.container(border=True):
        ...
        with col2:
            ...
            if st.button("Login", use_container_width=True, type="primary"):
                if name.strip():
                    if role == "Admin" and name.strip() not in ADMIN_USERS:
...
def admin_dashboard():
    ...
    with tab1:
        ...
        if df.empty:
            ...
            st.dataframe(df[['item_name', 'category', 'quantity', 'unit', 'created_by', 'timestamp']], hide_index=True, use_container_width=True)
        ...
        with col2:
            if st.button(" Clear All", type="primary", use_container_width=True):
                supabase.table("production").delete().neq('id', 0).execute()
    ...
    with tab2:
        ...
        if st.session_state.users:
            for name, data in list(st.session_state.users.items()):
                with st.expander(f"**{name}** ({data['role']}) - {', '.join(data.get('categories', [])) or 'No categories'}", expanded=False):
                    col1, col2 = st.columns([1, 3])
                    with col1:
                        new_role = st.selectbox("Role", ["Cook", "Admin"], 
                                            index=0 if data["role"] == "Cook" else 1, key=f"role_edit_{name}")
                    with col2:
                        new_cats = st.multiselect("Categories", list(MENU_DATA.keys()), 
                                              default=data.get("categories", []), key=f"cats_edit_{name}")
                    
                    col_btn1, col_btn2 = st.columns(2)
                    with col_btn1:
                        if st.button(" Update", key=f"update_user_{name}"):
                            if new_role == "Admin" and name not in ADMIN_USERS:
                                st.error(f"Cannot set Admin for {name}. Only ADMIN_USERS allowed.")
                                st.rerun()
                            st.session_state.users[name] = {"role": new_role, "categories": new_cats}
                            save_user(name, new_role, new_cats)
                            st.success(f"Updated {name}!")
                            st.rerun()
                    with col_btn2:
                        if st.button(" Delete", key=f"del_user_{name}"):
                            if name == st.session_state.user["name"] and st.session_state.user["role"] == "Admin":
                                st.error("Cannot delete currently logged-in admin.")
                                st.rerun()
        ...
        if st.button("Add User", type="primary", use_container_width=True):
            if new_name.strip():
                if new_role == "Admin" and new_name.strip() not in ADMIN_USERS:
                    st.error("🔒 Unauthorized: Only ADMIN_USERS can be set as Admin.")
                    st.rerun()
                if new_role == "Cook" and not new_cats:
                    st.error("Assign at least one category for Cook.")
                    st.rerun()
                save_user(new_name.strip(), new_role, new_cats)
                            ...
                            if st.button(" Update", key=f"edit_entry_{idx}"):
                                ...
                                for i, e in enumerate(st.session_state.production_data):
                                    if e['timestamp'] == entry['timestamp'] and e['created_by'] == selected_cook:
                                        ...
                                        break
                                supabase.table("production").upsert([st.session_state.production_data[i]]).execute()
                                st.success("Entry updated!")
                                st.rerun()
                        with col_delete:
                            if st.button(" Delete", key=f"del_entry_{idx}"):
                                st.session_state.production_data = [e for e in st.session_state.production_data 
                                                                 if not (e['timestamp'] == entry['timestamp'] and e['created_by'] == selected_cook)]
                                supabase.table("production").delete().eq('timestamp', entry['timestamp']).eq('created_by', selected_cook).execute()
                                st.success("Entry deleted!")
                                st.rerun()
            else:
                st.info("No cooks have entries yet.")
    ...
    with tab4:
        ...
        if not st.session_state.production_data:
            ...
            if filtered_df.empty:
                ...
                monthly = filtered_df.groupby(filtered_df['timestamp'].dt.to_period('M').astype(str))[['quantity']].agg({'quantity': 'sum'}).reset_index()
    ...
    with tab5:
        ...
        # Heatmap
        st.markdown("###  Production Heatmap (Day vs Hour)")
        ...
        fig_heat = px.imshow(heatmap_df, title="Production Heatmap", aspect="auto", color_continuous_scale='Viridis')
        ...
        fig_forecast.add_trace(go.Scatter(x=[tomorrow], y=[pred], mode='markers', name=f'Tomorrow: {pred:.2f}', marker=dict(size=12, color='red')))
